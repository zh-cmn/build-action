...
for FLAVOUR in "${FLAVOURS[@]}"; do
    for MODE in "${BOOTMODES[@]}"; do
        ...
        if [ "$FLAVOUR" = "gnome" ]; then
            ...
            chroot rootdir bash -c "export DEBIAN_FRONTEND=noninteractive && apt-get install -y kde-standard sddm firefox-esr akregator- dragonplayer- gwenview- juk- kaddressbook- kcalc- kmail- konq-plugins- korganizer- okular- keditbookmarks- konqueror- kwrite- partitionmanager-"
